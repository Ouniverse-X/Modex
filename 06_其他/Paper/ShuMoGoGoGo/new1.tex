...
\textbf{针对问题四，}利用实测半径建立\textbf{均匀仿射收缩模型}，引入材料坐标 $\xi=r/R(t)$ 将移动区域映射为固定区间，结合附录 4 物性关系，以表层加密有限体积法和隐式 BDF 求解，得到烘干时长为 $\mathbf{51.0906\,h}$，终止半径约为 $1.2000\,\mathrm{cm}$。干质量检验显示，真实湿密度解释下主模型的干质量检验量较初始减少约 $10.97\%$。为追溯其来源，构造\textbf{实测半径--反演长度}与\textbf{固定长度--预测半径}两个守恒对照模型，分别检验固定长度假设及几何观测与物性关系的相容性。前者产生显著轴向伸长，后者系统性高估半径，说明这些约束不能同时满足，从而识别模型结构矛盾并界定主模型时长的适用条件。
...
\subsection{问题二}
问题二沿用问题一建立的一维径向“传热-传质”模型，但进一步考虑药材物性参数随温度和含水率变化的影响。如图\ref{fig:q2-flowchart}所示，本文依据物性参数的状态相关关系构建热湿耦合求解流程。其中，密度、比热容和导热系数由含水率决定，水分扩散系数则同时受含水率和绝对温度影响，因此温度场与水分场通过变物性参数形成双向耦合。模型从烘干初始状态开始计算，环境温度和水分浓度边界沿用分段线性插值结果。求解时采用内部较疏、表面加密的两区非均匀径向有限体积网格，空间离散后形成刚性非线性常微分方程组，然后采用自适应变阶隐式BDF方法进行时间积分，不断同步更新各节点物性参数。
...
问题四进一步考虑药材在干燥过程中的径向收缩，要求重新计算达到全域干燥判据所需的时间。在问题三长期热湿耦合模型的基础上，本文首先对附件2给出的离散半径数据做分段线性插值处理并采用均匀仿射收缩描述各层随半径同步缩放的运动。为便于处理移动边界，引入随材料骨架运动的无量纲坐标$\xi=r/R(t)$，将区域$0\le r\le R(t)$映射到固定区间$0\le\xi\le1$，再结合附录4的变物性关系，将温度场、水分场及移动表面边界统一变换到收缩坐标系中。空间方向采用表层加密的有限体积离散，时间方向“采用自适应阶数与步长的隐式 BDF 方法推进热湿耦合状态，并以全域最大含水率首次达到阈值的时刻作为问题四的烘干完成时间。
...
根据 Fourier 导热定律，药材内部的径向热流密度 $q_r$ 为 $-k\frac{\partial T}{\partial r}$。如图\ref{fig:q1-control-volume}所示，本文取半径为 $r$、厚度为 $\mathrm{d}r$、长度为 $L$ 的圆柱薄壳作为控制体，其体积 $\mathrm{d}V$为$2\pi rL\,\mathrm{d}r$。
...
\subsubsection{水分场模型}
根据 Fick 第一定律\upcite{fick1855}，药材内部的径向水分扩散通量 $J_r$ 为 $-D(C)\frac{\partial C}{\partial r}$。对同一圆柱薄壳控制体，含水率的积累量为 $2\pi rL\,\mathrm{d}r\frac{\partial C}{\partial t}$，水分的净扩散输入量为 $-2\pi L\frac{\partial(rJ_r)}{\partial r}\,\mathrm{d}r$。
...
为进一步反映含水率随干燥时间和径向位置的整体演化规律，如图\ref{fig:q3-moisture-surface}所示，本文绘制了水分浓度$C(t,r)$的三维时空曲面，并叠加干燥阈值平面$C_{\mathrm{dry}}=0.15\,\mathrm{kg/kg}$。图中红色虚线为水分浓度曲面与阈值平面的交线，表征各径向位置达到干燥要求的时刻。随着干燥过程进行，各径向位置的水分浓度持续下降，靠近药材表面的区域较早达到干燥阈值，而中心区域下降相对滞后。

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/问题三三维曲面图.png}
\caption{水分浓度三维时空演化}
\label{fig:q3-moisture-surface}
\end{figure}

如图\ref{fig:q3-contour}所示，本文绘制了含水率时空等值线，以展示含水率在径向上的迁移过程及干燥达标区域的扩展。图中红色曲线为干燥阈值$C=0.15$对应的达标边界，曲线右侧的浅绿色区域表示含水率已达到干燥要求。由图可见，干燥达标边界由药材表面向中心推进，圆心最晚进入达标区域，即为药材内部最晚达到干燥要求的位置。

\begin{figure}[H]
\centering
\includesvg[width=\textwidth,inkscapelatex=false]{figures/问题三等值线图}
\caption{问题三含水率等值线及干燥达标边界}
...
设药材在时刻$t$的半径为$R(t)$。附件2相邻测点间作局部分段线性插值
\begin{equation}
...
空间离散沿用前文的守恒有限体积思想，在$[0,0.9]$与$[0.9,1]$采用步长比为$10:1$的表层加密网格。正式计算取$N=6080$个径向区间，使用隐式BDF方法\cite{hairer1996}推进温湿耦合状态；相对容差为$5\times10^{-12}$，温度和含水率绝对容差分别为$5\times10^{-12}$和$5\times10^{-14}$。定义
\begin{equation}
...
\subsection{第四问主模型的干质量守恒检验}

\subsubsection{干质量检验量的构造}

干基含水率的定义为$C=\mathrm dm_w/\mathrm dm_d$，故$\mathrm dm_{\mathrm{wet}}=(1+C)\mathrm dm_d$。若附录4的$\rho_w(C)=760+90C$是当前湿体积的真实密度，则单位当前体积中的干固体质量必为
\begin{equation}
\boxed{
...
=278.7323944\,\mathrm{kg/m^3},
\qquad
M_{d0}=\pi R_0^2L_0\rho_{d0}
=0.08756636\,\mathrm{kg}.
\label{eq:q4-check-initial-mass}
\end{equation}
...
\end{equation}
在上述临界时刻，数值积分得到$M_d^*(t_*^{(4)})/M_{d0}\approx0.8903$，即相对初值减少约$10.97\%$。【这并不表示真实药材必然损失了$10.97\%$的干物质，而是说明主模型的“实测半径、固定长度、仿射收缩和真实湿密度”不能共同代表同一个严格守恒的材料运动。】

\subsubsection{矛盾并非由数值离散造成}
...
\end{equation}
然而附件2在$24\,\mathrm h$给出的半径已为$1.204\,\mathrm{cm}$，在$72\,\mathrm h$进一步减至$1.198\,\mathrm{cm}$，均小于式\eqref{eq:q4-check-radius-lower-bound}。因此，真正不能完全相容的是
\[
\boxed{\text{真实湿密度}+\text{实测外半径}+\text{干质量守恒}+\text{长度不增加}}.
\]

\subsection{两个守恒对照模型}

为判断应放松哪一项几何约束，本文进一步构造两个非仿射守恒模型。两者均保留附录4给出的真实湿密度（即含水药材总质量与当前湿体积之比）和严格干质量守恒。

\subsubsection{共同的累计干质量坐标}

令$\mu=m_d/M_d\in[0,1]$为归一化累计干质量坐标，则固定的$\mu$区间始终代表同一部分干固体。对当前圆柱环层有
\begin{equation}
M_d\,\mathrm d\mu
=2\pi L(t)\rho_d(C)r\,\mathrm dr,
\label{eq:q4-mass-coordinate}
\end{equation}
故逐层几何映射和总体积约束分别为
\begin{equation}
...
\label{eq:q4-common-geometry}
\end{equation}
式\eqref{eq:q4-mass-coordinate}把局部干质量守恒直接写入材料坐标。依据Fick定律和Fourier定律\cite{fick1855,crank1975,bird2002}，两个对照模型共同采用
\begin{equation}
\frac{\partial C}{\partial t}
=\frac{\partial}{\partial\mu}
\left(K_C\frac{\partial C}{\partial\mu}\right),
\qquad
(1+C)c_p(C)\frac{\partial T}{\partial t}
=\frac{\partial}{\partial\mu}
\left(K_T\frac{\partial T}{\partial\mu}\right),
\label{eq:q4-mass-coordinate-pde}
\end{equation}
其中
\begin{equation}
K_C=\frac{4\pi^2L(t)^2r^2\rho_d(C)^2D}{M_d^2},
\qquad
K_T=\frac{4\pi^2L(t)^2r^2\rho_d(C)k}{M_d^2}.
\label{eq:q4-mass-coordinate-coefficients}
\end{equation}
初始条件、环境过程及表面对流系数与问题四模型相同。

\subsubsection{对照模型一：输入实测半径，反演长度}

第一个对照模型保留附件2的$R_{\mathrm{obs}}(t)$，但不再固定长度。由式\eqref{eq:q4-common-geometry}得到
\begin{equation}
\boxed{
...
\end{equation}
以$N=3040$、$\Delta t=1\,\mathrm s$计算得到临界时刻$47.8764\,\mathrm h$；反演长度由$25\,\mathrm{cm}$最高增至$32.3855\,\mathrm{cm}$，临界时刻仍为$28.0723\,\mathrm{cm}$。该方案在数学上同时满足真实密度、实测半径和逐环干质量守恒，但代价是产生了未经观测支持的显著轴向伸长。

\begin{figure}[H]
\centering
\includegraphics[width=0.76\textwidth]{../../problem4-2/experiments/true_density_nonaffine_20260911_phase3_center_reconstructed/figures/geometry_diagnostics.png}
\caption{输入实测半径后由干质量守恒反演得到的长度及非仿射程度}
\label{fig:q4-inferred-length}
\end{figure}

\subsubsection{对照模型二：固定长度，独立预测半径}

第二个对照模型固定$L=L_0$，只把附件2半径作为后验。外半径由式\eqref{eq:q4-common-geometry}唯一预测为
\begin{equation}
\boxed{
R_{\mathrm{pred}}(t)
=R_0\left[
...
\end{equation}
参考计算得到$R_{\mathrm{pred}}(72\,\mathrm h)=1.264861\,\mathrm{cm}$，而实测值为$1.198000\,\mathrm{cm}$；145个测量时刻的$\mathrm{MAE}=0.109782\,\mathrm{cm}$、$\mathrm{RMSE}=0.120537\,\mathrm{cm}$，除初始点外预测残差全部为正，最大误差$0.229165\,\mathrm{cm}$出现在$6\,\mathrm h$。该模型的临界时刻为$56.2103\,\mathrm h$。

\begin{figure}[H]
\centering
\includegraphics[width=0.94\textwidth]{../../problem4-3/experiments/fixed_length_free_radius_20260911_final/figures/radius_attachment2_comparison.pdf}
\caption{固定长度守恒模型的半径预测与附件2实测值比较}
\label{fig:q4-radius-validation}
\end{figure}

如图\ref{fig:q4-radius-validation}所示，本文将固定长度守恒模型的预测半径与附件2的实测半径进行了比较。除初始时刻外，预测半径始终显著大于实测半径，且偏差远超数值计算误差。在附录4真实湿密度和实测收缩数据均作为可信输入的前提下，这表明固定长度与严格干质量守恒不能同时满足实际收缩过程，至少有一项约束并不成立。因此，本文主模型在保留固定长度和实测半径的前提下放松干质量守恒，具有独立对照计算的依据，而非人为舍弃守恒条件。

\subsection{检验结果}

三种模型的作用和结论汇总于表\ref{tab:q4-model-comparison}。两个守恒对照分别检验放松长度约束和放松半径输入后会发生什么。

\begin{table}[H]
\centering
\renewcommand{\arraystretch}{1.25}
\caption{第四问三种闭合方式的结果与解释边界}
\label{tab:q4-model-comparison}
\begin{tabularx}{0.96\textwidth}{|>{\centering\arraybackslash}p{0.19\textwidth}|>{\centering\arraybackslash}p{0.13\textwidth}|>{\centering\arraybackslash}X|>{\centering\arraybackslash}X|}
\hline
方案 & 临界时刻 & 主要证据 & 主要局限\\
\hline
均匀仿射收缩主模型 & $51.0906\,\mathrm h$ & 直接采用附件2半径，完成题目要求的移动边界计算 & 若密度为真实湿密度，临界时刻干质量检验量约减少$10.97\%$\\
\hline
实测半径--反演长度 & $47.8764\,\mathrm h$ & 真实密度、实测半径和逐环干质量同时守恒 & 反演长度最高$32.3855\,\mathrm{cm}$，缺少轴向实测支持\\
\hline
固定长度--预测半径 & $56.2103\,\mathrm h$ & 附件2未进入正向求解，可作独立外部检验 & 半径RMSE为$0.120537\,\mathrm{cm}$，系统性高估收缩后半径\\
\hline
\end{tabularx}
\end{table}

因此，本文仍将直接使用附件2并采用均匀仿射收缩所得的$t_*^{(4)}\approx51.09\,\mathrm h$作为第四问正式答案；同时明确该结果是均匀仿射主模型下的工程基线，而非已经同时通过真实湿密度、几何观测和干质量守恒验证的唯一物理真值。另两个时长是结构敏感性结果，不能解释为统计置信区间，也不替代主答案。

\section{模型评价与展望}

\subsection{模型优点}
\begin{itemize}[leftmargin=2em,itemsep=2pt,topsep=2pt]
  \item \textbf{由简入繁，主次清晰：}先用均匀仿射收缩完成题目要求并得到$51.09\,\mathrm h$，再由守恒检验决定是否需要更复杂的几何描述，避免一开始引入无法辨识的自由度。
  \item \textbf{物理检验可追溯：}干质量矛盾由含水率定义、真实湿密度和圆柱体积直接推出；累计干质量坐标又将逐环守恒写成几何恒等式，结论不依赖单一求解器的偶然输出。
  \item \textbf{区分数值误差与结构误差：}主模型完成网格和时间加密；固定长度对照的半径数值差值约为$10^{-6}\,\mathrm{cm}$量级，而模型--数据偏差约为$10^{-1}\,\mathrm{cm}$量级，因此能够判断主要问题来自闭合假设而非网格精度。
  \item \textbf{对照模型具有可解释性：}一个模型把冲突显化为异常的反演长度，另一个模型把冲突显化为可直接与附件2比较的半径偏差，使每项额外假设的代价都可以被观察和讨论。
\end{itemize}

\subsection{模型局限}
\begin{itemize}[leftmargin=2em,itemsep=2pt,topsep=2pt]
  \item 主模型直接利用实测半径，但在固定长度与真实湿密度解释下不能严格保持干质量，因此$51.09\,\mathrm h$带有明确的结构条件。
  \item 反演长度模型缺少长度实测约束，预测出的轴向伸长不能视为已经验证的真实变形；固定长度模型虽严格守恒，却系统性高估附件2半径。
  \item 三种模型均以一维径向场为主，尚未联合描述端面传热传质、轴向应变、孔隙率变化、组织压实与开裂等可能影响宏观体积的机制。
  \item 温度方程未显式考虑水分蒸发吸收的汽化潜热，也未考虑体热源、化学反应热、辐射换热及热扩散引起的直接水分通量，相关影响作为模型误差处理。
\end{itemize}

\subsection{进一步改进方向}
后续实验应同步测量$R(t)$、$L(t)$、样品总质量及代表时刻的真实体积密度或孔隙率。只需增加独立的长度或体积信息，便可判断偏差主要来自轴向变形还是密度--孔隙关系。数据充分后，可在累计干质量坐标基础上引入轴向伸缩比和孔隙率状态变量，并建立二维轴对称$(r,z)$传热传质模型，对侧面与端面分别标定换热、传质系数。这样才能在严格质量守恒的同时利用半径观测，并把第四问的条件性工程估计进一步提升为可由实验辨识和验证的物理模型。
