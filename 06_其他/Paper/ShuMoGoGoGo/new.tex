...
\begin{abstract}
中药材在加工后需要及时脱除水分，以提高储藏稳定性并保持成品品质，利用热空气脱除水分是生产中常用的处理方式。本文以圆柱形药材为对象，将烘房实测温湿度作为动态边界，依次建立固定域、变物性耦合及收缩移动边界模型，求解预热与恒温干燥阶段的温度场和干基含水率场，并确定全域含水率达标所需时间。

\textbf{针对问题一，}将细长圆柱简化为一维轴对称介质，依据 Fourier 定律、Fick 定律和控制体守恒建立非稳态传热传质方程及表面对流边界，采用\textbf{守恒型径向有限体积法、后向 Euler 全隐式格式和 Picard 迭代}求解。$1800\,\mathrm{s}$时中心与表面温度分别为$33.5753\,{}^\circ\mathrm{C}$和$36.7856\,{}^\circ\mathrm{C}$，水分浓度分别为$2.5500\,\mathrm{kg/kg}$和$1.5102\,\mathrm{kg/kg}$，表明表面响应显著快于内部。

\textbf{针对问题二，}引入随含水率变化的密度、比热容和导热系数，以及同时受温度和含水率影响的扩散系数，构建\textbf{变物性热湿双向耦合模型}；采用表层加密的两区非均匀网格和\textbf{自适应变阶隐式 BDF 方法}求解刚性半离散系统。$3\,\mathrm{h}$时中心与表面温度为$49.8495\,{}^\circ\mathrm{C}$和$49.9664\,{}^\circ\mathrm{C}$，水分浓度为$1.7662\,\mathrm{kg/kg}$和$1.0081\,\mathrm{kg/kg}$，说明温度已接近平衡而水分梯度仍然显著。

\textbf{针对问题三，}沿用固定半径变物性模型，将观测区间后的边界连续过渡至稳定环境，并引入全域最大含水率构造烘干判据。利用\textbf{表层加密有限体积--隐式 BDF 框架}追踪中心阈值，求得连续临界时刻为$\mathbf{57.4723\,h}$，含水率场综合经验误差为$9.53\times10^{-7}\,\mathrm{kg/kg}$。空间--时间独立加密、\textbf{Chebyshev 谱元--Radau IIA 异构复算}及\textbf{Bessel 解析解检验}共同验证了结果的数值稳定性。

\textbf{针对问题四，}将实测半径作为移动边界，建立\textbf{均匀仿射收缩--材料坐标模型}，以$\xi=r/R(t)$将时变区域映射至固定区间，并结合附录4物性关系求解温湿场。所得烘干临界时刻为$\mathbf{51.0906\,h}$，此时半径约为$1.2000\,\mathrm{cm}$，较问题三缩短$6.3817\,\mathrm{h}$；该差异同时包含收缩与物性变化。进一步利用干质量一致性诊断及两个守恒对照模型，揭示了实测几何、密度解释与干质量守恒间的结构不确定性。

本文以统一守恒框架贯通固定域与移动域计算，并通过\textbf{表面加密、刚性隐式积分、事件定位、独立收敛与跨算法复算}构成数值证据链，同时区分离散误差和模型结构误差，为药材烘干过程预测与工艺评价提供了可解释工具。

\keywords{热风烘干\quad 热湿耦合\quad 有限体积法\quad 移动边界\quad 数值验证}
\end{abstract}
...
\subsection{问题四}
问题四首先只考虑题目要求的收缩效应：将附件2的实测半径作为移动边界，假设圆柱长度不变且径向作均匀仿射收缩，在固定材料坐标中求解附录4物性下的热湿耦合方程，得到烘干临界时长。得到主结果后，再以干基含水率和真实湿密度的定义检验干固体质量，判断主模型的几何、密度与守恒关系是否相容。针对检验中发现的矛盾，进一步建立“实测半径--反演长度”和“固定长度--预测半径”两个非仿射守恒模型，分别考察放松长度约束或放松实测半径约束后的结果及代价。由此形成“主模型计算--守恒检验--对照修正--模型评价”的递进分析。
...
\begin{enumerate}[label=\arabic*.,leftmargin=2em,itemsep=0pt,topsep=0pt]
  ...
  \item \textbf{第四问主模型的收缩方式：}主模型假设圆柱长度保持$L_0$不变，同一初始相对半径处的材料层始终具有相同的径向缩放比，即作均匀仿射收缩。该假设仅用于先行求得题设驱动的工程基线，随后将由干质量守恒检验其适用性。
...
附件1给出的烘房环境数据覆盖$0\le t\le t_a=14400\,\mathrm{s}$。在该区间内保留全部测量点，并按问题一的方法作分段线性插值。题面没有给出$4\,\mathrm h$后的环境数据，而求得全域达标时刻必须延长边界输入。为避免在$t_a$处人为产生跳变，本文声明如下长期闭合：从附件末值出发，在$\Delta t_a=60\,\mathrm{s}$内线性过渡到稳定烘干环境$50\,{}^\circ\mathrm C$和$0.05\,\mathrm{kg/kg}$，随后保持不变。令$\lambda=(t-t_a)/\Delta t_a$，则
\begin{equation}
...
&0\le t\le t_a,\\
(1-\lambda)\bigl(T_{\infty,\mathrm{att}}(t_a),C_{\infty,\mathrm{att}}(t_a)\bigr)
+\lambda\bigl(50,0.05\bigr),
&t_a<t<t_a+\Delta t_a,\\
\bigl(50,0.05\bigr),
&t\ge t_a+\Delta t_a.
\end{cases}
...
\end{equation}
式\eqref{eq:q3-environment-extension}中的过渡段和稳定值属于模型闭合而非实测数据；本文所有第三问主解及辅助算法均使用这一相同输入，避免将边界延拓差异混入算法比较。
...
\end{equation}
其单位依次为$\mathrm{kg/m^3}$、$\mathrm{J/(kg\cdot K)}$、$\mathrm{W/(m\cdot K)}$和$\mathrm{m^2/s}$。扩散系数中的温度必须取绝对温度。环境函数沿用式\eqref{eq:q3-environment-extension}：附件1覆盖范围内作分段线性插值，$4\,\mathrm h$后采用末$1\,\mathrm h$平台均值的稳定工况代表值$50\,{}^\circ\mathrm C$和$0.05\,\mathrm{kg/kg}$。
...
本问先建立一个直接利用附件2半径数据的主模型。设药材在时刻$t$的半径为$R(t)$，长度保持初值$L_0=0.25\,\mathrm m$。附件2共有145个测点，覆盖$0\le t\le72\,\mathrm h$；相邻测点间只作局部分段线性插值
\begin{equation}
R(t)=R_j+\frac{t-t_j}{t_{j+1}-t_j}\bigl(R_{j+1}-R_j\bigr),
\qquad t_j\le t\le t_{j+1}.
\label{eq:q4-main-radius}
\end{equation}
式\eqref{eq:q4-main-radius}仅用于恢复离散测量之间的连续边界，并不假定72 h内的整体收缩规律为线性。本文所得临界时刻位于附件2观测区间内，因而不需要外推半径。
...
\label{eq:q4-affine-map}
\end{equation}
因此不同材料层不会相互穿越，初始同心圆在收缩后仍为同心圆。取无量纲材料坐标
\begin{equation}
\xi=\frac{r}{R(t)}=\frac{r_0}{R_0},
\qquad 0\le\xi\le1,
\label{eq:q4-main-xi}
\end{equation}
并记$\Theta(\xi,t)=T(\xi R(t),t)$、$W(\xi,t)=C(\xi R(t),t)$。由式\eqref{eq:q4-affine-map}，材料骨架速度为$u_r=(\dot R/R)r$；对任意随骨架运动的场$f$，链式法则给出
\begin{equation}
\frac{\mathrm Df}{\mathrm Dt}
=\left.\frac{\partial f}{\partial t}\right|_r
+u_r\frac{\partial f}{\partial r}
...
\end{equation}
这表明采用材料坐标后，网格运动项已经包含在式\eqref{eq:q4-main-material-derivative}中，不应重复添加收缩对流项。

\subsubsection{收缩坐标下的热湿耦合模型}

主模型沿用前两问的一维径向传热传质结构，并从$t=0$起采用附录4给出的物性关系。圆柱径向扩散算子在$r=\xi R(t)$下产生尺度因子$R(t)^{-2}$，故固定区间$\xi\in[0,1]$上的控制方程为
\begin{equation}
\boxed{
...
\end{equation}
式\eqref{eq:q4-main-properties}依次采用$\mathrm{kg/m^3}$、$\mathrm{J/(kg\cdot K)}$、$\mathrm{W/(m\cdot K)}$和$\mathrm{m^2/s}$为单位。
...
\end{equation}
其中$h=25\,\mathrm{W/(m^2\cdot K)}$、$h_m=8\times10^{-7}\,\mathrm{m/s}$。环境过程沿用式\eqref{eq:q3-environment-extension}，从而保证第三、四问长期边界的比较口径一致。
...
\subsubsection{主模型计算结果}

数值计算得到连续临界时刻
\begin{equation}
\boxed{
t_*^{(4)}=183926.094367684\,\mathrm s
=51.0905817688\,\mathrm h
\approx51.09\,\mathrm h.
}
\label{eq:q4-main-result}
\end{equation}
由于临界时刻处$C_{\max}^{(4)}=0.15$，严格不等式在任意略晚于$t_*^{(4)}$的时刻成立。典型时刻的含水率和实测半径见表\ref{tab:q4-main-moisture}。

\begin{table}[H]
\centering
\renewcommand{\arraystretch}{1.16}
\caption{均匀仿射收缩主模型的含水率计算结果}
...
与问题三固定半径模型的$57.4723\,\mathrm h$相比，主模型的临界时刻缩短约$6.3817\,\mathrm h$。该差异同时包含半径收缩和附录4物性变化，不能全部解释为单一的几何收缩效应；就方程结构而言，半径减小通过式\eqref{eq:q4-main-heat}和式\eqref{eq:q4-main-moisture}中的$R(t)^{-2}$加快了内部传递。

\section{模型检验及合理性分析}

\subsection{第三问模型的数值检验}

\subsubsection{空间--时间收敛性}
...
\end{equation}
谱元临界时刻为$206900.213243\,\mathrm s$，与有限体积结果的差异小于式\eqref{eq:q3-time-error-estimate}的经验估计。由此可认为，第三问的径向场分布及四位小数时长没有表现出明显的单一空间离散或时间推进依赖；这一结论仍只属于共同模型下的数值交叉证据，不能替代物理实验验证。综上，有限体积自收敛与谱元同模型复算分别提供了数值稳定性和跨离散一致性证据，第三问正式报告$t_{\mathrm{rep}}=57.4723\,\mathrm h$。
...
主模型已经给出清晰的数值答案，但还需检查其几何收缩与附录4真实湿密度能否共同满足物质守恒。干基含水率的定义为$C=\mathrm dm_w/\mathrm dm_d$，故$\mathrm dm_{\mathrm{wet}}=(1+C)\mathrm dm_d$。若附录4的$\rho_w(C)=760+90C$是当前湿体积的真实密度，则单位当前体积中的干固体质量必为
\begin{equation}
\boxed{
\rho_d(C)=\frac{\rho_w(C)}{1+C}
=\frac{760+90C}{1+C}.
...
\end{equation}
式\eqref{eq:q4-check-dry-density}只是含水率与体积密度定义的恒等变换，并非新的经验假设。初始状态均匀，因此
\begin{equation}
...
把主模型计算出的$W(\xi,t)$、实测半径$R(t)$和固定长度$L_0$代入真实密度定义，可得到后验干质量检验量
\begin{equation}
M_d^*(t)=2\pi L_0R(t)^2
\int_0^1\frac{760+90W(\xi,t)}{1+W(\xi,t)}\,\xi\,\mathrm d\xi.
\label{eq:q4-check-mass-proxy}
\end{equation}
在式\eqref{eq:q4-main-result}的临界时刻，数值积分得到$M_d^*(t_*^{(4)})/M_{d0}\approx0.8903$，即相对初值减少约$10.97\%$。这并不表示真实药材必然损失了$10.97\%$的干物质，而是说明主模型的“实测半径、固定长度、仿射收缩和真实湿密度”不能共同代表同一个严格守恒的材料运动。

\subsubsection{矛盾并非由数值离散造成}

上述结论还可不依赖主模型的数值场而直接证明。由式\eqref{eq:q4-check-dry-density}，对$C\ge0$恒有$\rho_d(C)\le760\,\mathrm{kg/m^3}$。若总干质量守恒且长度不超过初始长度，则
\begin{equation}
...
\]
该必要条件没有使用仿射映射或扩散方程，所以仅取消均匀仿射收缩、细化网格或改变临界时刻求解器，都不能消除这一结构矛盾。

\subsection{真实湿密度下的两个守恒对照模型}

为判断应放松哪一项几何约束，本文进一步构造两个非仿射守恒模型。两者均保留附录4真实湿密度和严格干质量守恒，但分别放松“长度固定”和“外半径必须作为输入”。
...
\end{equation}
初始条件、环境过程及表面对流系数与主模型相同。式\eqref{eq:q4-mass-coordinate-pde}采用单位干质量形式，因而无需另设所谓“收缩浓缩项”。
...
\end{equation}
以$N=3040$、$\Delta t=1\,\mathrm s$计算得到临界时刻$47.8764\,\mathrm h$；反演长度由$25\,\mathrm{cm}$最高增至$32.3855\,\mathrm{cm}$，临界时刻仍为$28.0723\,\mathrm{cm}$。该方案在数学上同时满足真实密度、实测半径和逐环干质量守恒，但代价是产生了未经观测支持的显著轴向伸长。由于题目没有给出长度时序，反演长度只能视为“使三项条件相容所需的等效几何量”，不能直接宣称为药材的真实变形。
...
第二个对照模型固定$L=L_0$，不把附件2半径输入正向求解。外半径由式\eqref{eq:q4-common-geometry}唯一预测为
\begin{equation}
\boxed{
R_{\mathrm{pred}}(t)
=R_0\left[
...
\end{equation}
附件2只在求解完成后用于独立验证。参考计算得到$R_{\mathrm{pred}}(72\,\mathrm h)=1.264861\,\mathrm{cm}$，而实测值为$1.198000\,\mathrm{cm}$；145个测量时刻的$\mathrm{MAE}=0.109782\,\mathrm{cm}$、$\mathrm{RMSE}=0.120537\,\mathrm{cm}$，除初始点外预测残差全部为正，最大误差$0.229165\,\mathrm{cm}$出现在$6\,\mathrm h$。该模型的临界时刻为$56.2103\,\mathrm h$。
...
图\ref{fig:q4-radius-validation}显示，模型预测曲线在整个非初始观测区间均高于实测曲线，说明固定长度、真实湿密度和干质量守恒虽然彼此数学相容，但所预测的体积收缩不足。也就是说，该方案的弊端不是无法求解，而是不能充分复现实测收缩过程。

\subsection{检验结果与主答案取舍}

三种模型的作用和结论汇总于表\ref{tab:q4-model-comparison}。两个守恒对照不是为了从三个时长中挑选一个更“好看”的结果，而是分别检验放松长度约束和放松半径输入后会发生什么。
...
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.21\textwidth}>{\raggedright\arraybackslash}p{0.22\textwidth}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X}
\hline
方案 & 临界时刻 & 主要证据 & 主要局限\\
\hline
...
为排除“主结果只是离散误差”的可能性，本文对主模型使用$N=1520,3040,6080$三层相似网格。相邻两次加密的含水率最大差分别为$6.945758\times10^{-6}$和$1.736444\times10^{-6}\,\mathrm{kg/kg}$，观测收敛阶为$1.999997$；带$1.25$安全系数的细网格Richardson经验误差估计为$7.235205\times10^{-7}\,\mathrm{kg/kg}$\cite{roache1998}。在$N=6080$上进一步收紧时间积分后，临界时刻仅变化$5.47\times10^{-6}\,\mathrm s$。因此$51.09\,\mathrm h$在给定主模型内部具有充分的数值稳定性，但这种数值收敛不能消除式\eqref{eq:q4-check-radius-lower-bound}揭示的模型结构矛盾。

综合题目作答目标，本文仍将直接使用附件2并采用均匀仿射收缩所得的
\[
\boxed{t_*^{(4)}\approx51.09\,\mathrm h}
\]
作为第四问正式答案；同时明确该结果是均匀仿射主模型下的工程基线，而非已经同时通过真实湿密度、几何观测和干质量守恒验证的唯一物理真值。另两个时长是结构敏感性结果，不能解释为统计置信区间，也不替代主答案。
...
\bibitem{bird2002}
Bird R B, Stewart W E, Lightfoot E N. Transport Phenomena[M]. 2nd ed. New York: Wiley, 2002.
