...
\begin{abstract}
中药材在加工后需要及时脱除水分，以提高储藏稳定性并保持成品品质，利用热空气脱除水分是生产中常用的处理方式。本文以圆柱形药材为对象，围绕预热升温、变物性热湿耦合与失水收缩，逐步建立径向传热传质模型，预测内部温度和干基含水率的时空分布，确定满足全域干燥要求的烘干时长。
...
\textbf{针对问题四，}利用实测半径建立\textbf{均匀仿射收缩模型}，引入材料坐标 $\xi=r/R(t)$ 将移动区域映射为固定区间，结合附录 4 物性关系，以表层加密有限体积法和隐式 BDF 求解，得到烘干时长为 $\mathbf{51.0906\,h}$，终止半径约为 $1.2000\,\mathrm{cm}$。后验检验表明，临界时刻样品控制体内的有效干质量相对初始值下降约 $10.97\%$。进一步将干质量严格恒定作为反设，构造\textbf{实测半径--反演长度}与\textbf{固定长度--预测半径}两个守恒对照模型；前者要求产生显著轴向伸长，后者系统性高估药材半径，分别从几何合理性和观测一致性方向支持主模型的有效干质量非恒定结论。

本文将固定域与收缩域模型逐步衔接，通过\textbf{网格与时间收敛、跨算法复算和干质量一致性检验}，分别评估数值离散误差与模型结构局限，为药材干燥过程预测及后续实验设计提供依据。

\keywords{热风烘干\quad 热湿耦合\quad 有限体积法\quad 移动边界\quad 模型检验}
\end{abstract}
...
\subsection{问题三}
问题三沿用问题二的控制方程及初边值条件。由于附件1仅给出前$4\,\mathrm{h}$的环境数据，需在该时段内进行分段线性插值，并以第$3\sim4\,\mathrm{h}$环境数据的平均值对后续环境边界条件作常值延拓。本文以全域最大含水率作为状态指标，将其首次降至干燥阈值的时刻定义为烘干完成时刻。数值求解时对表层区域加密，空间方向采用径向有限体积法，时间方向首步采用后向Euler格式、后续采用可变步长BDF2格式，并在每个时间层内通过Picard迭代更新温度场和含水率场，最后定位阈值穿越时刻，得到长期含水率分布和烘干时间。
...
\centering
\includesvg[width=\linewidth,inkscapelatex=false]{figures/温度三维时空分布曲面图}
\caption{温度三维时空分布曲面图}
...
\begin{subfigure}[b]{0.48\textwidth}
\centering
\includesvg[width=\linewidth,inkscapelatex=false]{figures/水分浓度三维时空分布曲面图}
...
为定量比较不同时刻和径向位置处的计算结果，本文从数值解中提取$t=100,300,600,900,1200,1500,1800\,\mathrm{s}$时，$r=0,0.5,1.0,1.5,2.0\,\mathrm{cm}$处的温度和水分浓度，结果分别列于表\ref{tab:q1-temperature}和表\ref{tab:q1-moisture}。
...
\paragraph{网格计算与误差验证}
为检验非均匀网格的充分性，保持表面加密层厚度和$10$倍加密比例不变，依次采用$N=190,380,760$进行空间加密。由三层结果得到的温度、含水率最大范数观测阶分别为$1.8790$和$1.9999$，细网格的$1.25$倍 Richardson 安全误差上界\upcite{roache1998}分别为$4.665153\times10^{-6}\,{}^\circ\mathrm C$和$1.455194\times10^{-5}\,\mathrm{kg/kg}$。在$N=760$上进一步将积分容差缩小$4$倍并把最大内部步长减至$2.5\,\mathrm{s}$，温度和含水率最大变化分别为$5.360042\times10^{-6}\,{}^\circ\mathrm C$和$4.365391\times10^{-8}\,\mathrm{kg/kg}$。结果表明，表面加密网格和自适应时间积分均已达到题目四位小数输出所需的稳定精度。
...
\begin{figure}[H]
\centering
\includesvg[width=0.9\textwidth,inkscapelatex=false]{figures/问题二新结果展示}
\caption{问题二温度场与水分浓度场计算结果}
\label{fig:q2-results}
\end{figure}

为定量比较不同时刻和径向位置处的计算结果，本文提取$0.5,1.0,1.5,2.0,2.5,3.0\,\mathrm{h}$时，$r=0,0.5,1.0,1.5,2.0\,\mathrm{cm}$处的温度和水分浓度，结果分别列于表\ref{tab:q2-temperature}和表\ref{tab:q2-moisture}。
...
为进一步反映含水率随干燥时间和径向位置的整体演化规律，本文绘制水分浓度$C(t,r)$的三维时空曲面和时空等值线，结果如图\ref{fig:q3-moisture-evolution}所示。子图(a)叠加干燥阈值平面$C_{\mathrm{dry}}=0.15\,\mathrm{kg/kg}$，红色虚线为水分浓度曲面与阈值平面的交线，表征各径向位置达到干燥要求的时刻；子图(b)中的红色曲线为$C=0.15$对应的达标边界，其右侧浅绿色区域表示含水率已达到干燥要求。两种表示共同表明，随着干燥过程进行，各径向位置的水分浓度持续下降，干燥达标边界由药材表面向中心推进，圆心最晚达到干燥要求。
...
\begin{subfigure}[b]{0.49\textwidth}
\centering
\includesvg[width=\linewidth,inkscapelatex=false]{figures/问题三等值线图}
\caption{含水率等值线及干燥达标边界}
\label{fig:q3-contour}
\end{subfigure}
\caption{问题三含水率的时空演化及干燥达标过程}
...
数值计算覆盖完整干燥过程和全部径向区域，完整计算结果保存于文件\texttt{result3.xlsx}。根据全域干燥判据，连续临界时刻为$t_*^{(3)}=57.472300886994\,\mathrm{h}$；向上保留四位小数后，烘干时长取$t_{\mathrm{rep}}^{(3)}=57.4724\,\mathrm{h}\approx2.40\,\mathrm{d}$。
为定量比较长期干燥过程中不同时间和径向位置处的含水率，本文提取$6,12,\ldots,54\,\mathrm{h}$及报告终止时刻$t_{\mathrm{rep}}^{(3)}$时，$r=0,0.5,1.0,1.5,2.0\,\mathrm{cm}$处的含水率，结果见表\ref{tab:q3-moisture}。
...
空间离散沿用前文的守恒有限体积思想，在$[0,0.9]$与$[0,1]$采用步长比为$10:1$的表层加密网格。正式计算取$N=6080$个径向区间，使用隐式BDF方法\upcite{hairer1996}推进温湿耦合状态；相对容差为$5\times10^{-12}$，温度和含水率绝对容差分别为$5\times10^{-12}$和$5\times10^{-14}$。定义
\begin{equation}
...
问题四的数值计算覆盖$0\le t\le t_*^{(4)}$的完整干燥过程和$0\le\xi\le1$的全部径向节点，完整计算结果保存于文件\texttt{result4.xlsx}。连续临界时刻为$t_*^{(4)}=51.090576239622\,\mathrm{h}$，按题意保留四位小数后，烘干时长为$t_{\mathrm{dry}}^{(4)}=51.0906\,\mathrm{h}$。
...
为定量比较收缩过程中不同时间和径向位置处的含水率，本文提取$6,12,\ldots,48\,\mathrm{h}$及报告终止时刻$51.0906\,\mathrm h$时，距药材中心$0,0.5,1.0\,\mathrm{cm}$及药材表面处的含水率，结果见表\ref{tab:q4-main-moisture}。
...
\begin{figure}[H]
\centering
\includesvg[width=\textwidth,inkscapelatex=false]{figures/问题三等值线图}
\caption{问题三含水率场的时空等值线图}
\label{fig:q3-contours}
\end{figure}
...
对三级网格所得结果$Q_h,Q_{h/2},Q_{h/4}$，采用 Richardson 外推计算观测收敛阶，并以$1.5$为安全系数估计细网格解的剩余误差\cite{roache1998}：
\begin{equation}
\left\{
\begin{aligned}
p_{\mathrm{obs}}
...
在保持物理模型、参数及初边界条件一致的前提下，本文采用 Chebyshev--Lobatto 谱元法\cite{trefethen2000}和五阶 Radau IIA 方法\cite{hairer1996}进行独立复算。空间上，主方法采用有限体积法计算控制体界面通量，对照方法采用高阶多项式弱形式逼近；时间上，主方法采用BDF2格式和Picard迭代，对照方法采用五阶隐式配点格式。此外，同一Robin边界条件分别通过外层控制体通量和弱形式边界积分施加，临界时刻则分别通过离散节点插值和连续谱近似定位，从而对上述四个数值环节进行交叉检验。
...
\subsubsection{主模型的有效干质量检验}

干基含水率以干物质质量为基准，干质量是联系含水率、湿密度与收缩体积的核心量。为此，本文根据主模型所得含水率场，计算留存在当前药材样品控制体内的有效干质量及其变化。【？】
...
\end{equation}
在上述临界时刻，数值积分得到$M_d^*(t_*^{(4)})/M_{d0}\approx0.8903$，即样品控制体内有效干质量的后验相对降幅约为$10.97\%$。该结果表明，在主模型采用实测收缩体积、固定长度和真实湿密度关系的条件下，留存在当前药材样品控制体内的有效干质量不能继续视为恒定值。为判断这一降幅的合理性，下面将“干质量严格恒定”作为反设进行解析检验，并构造两个严格守恒对照模型。

\subsubsection{两个严格守恒对照模型}

为验证主模型所得有效干质量下降是否具有结构必然性，本文将干质量严格守恒作为反设，构造两个非仿射守恒对照模型。两个模型分别在保留实测半径和保持固定长度的情况下完成几何闭合，用于考察强制干质量守恒所产生的几何后果。

两个对照模型均采用归一化累计干质量坐标$\mu=m_d/M_d\in[0,1]$描述非仿射变形，使固定的$\mu$区间始终对应同一部分干固体，从而将逐环干质量守恒直接纳入计算。两者沿用问题四的初始条件、环境过程和传热传质边界，仅采用不同的几何闭合方式。

\paragraph{对照模型一：保留实测半径并反演长度}

对照模型一强制总干质量和逐环干质量严格守恒，保留附件2的实测半径$R(t)$，放开长度约束，并利用非仿射累计干质量坐标反演$L(t)$，得到
\begin{equation}
\boxed{
...
\end{equation}
以$N=3040$、$\Delta t=1\,\mathrm s$计算得到临界时刻$47.8764\,\mathrm h$；反演长度由$25\,\mathrm{cm}$最高增至$32.3855\,\mathrm{cm}$，最大轴向伸长率为$(32.3855-25)/25\times100\%\approx29.54\%$。该模型在数学上可以同时满足题给密度关系、实测半径和逐环干质量守恒，但必须产生显著的整体轴向伸长。药材未受到外部轴向拉伸作用，自由干燥过程通常以组织收缩、塌陷和翘曲为主，最高约$29.54\%$的整体轴向伸长缺少合理的力学来源。该结果从几何合理性角度说明问题四主模型中有效干质量减少具有合理性。

\begin{figure}[H]
\centering
\includesvg[width=0.76\textwidth,inkscapelatex=false]{figures/输入实测半径后由干质量守恒反演得到的长度及非仿射程度}
\caption{对照模型一由干质量守恒反演得到的长度及非仿射程度}
\label{fig:q4-inferred-length}
\end{figure}

\paragraph{对照模型二：保持固定长度并预测半径}

对照模型二同样强制总干质量和逐环干质量严格守恒，并固定长度，不将附件2半径输入正向求解，而由守恒关系独立预测外半径；附件2的半径数据仅作为后验检验依据。预测半径为
\begin{equation}
\boxed{
R_{\mathrm{pred}}(t)
=R_0\left[
\rho_{d0}\int_0^1\frac{1}{\rho_d(C(\mu,t))}\,\mathrm d\mu
\right]^{1/2}.
}
\label{eq:q4-predicted-radius}
\end{equation}
参考计算得到$R_{\mathrm{pred}}(72\,\mathrm h)=1.264861\,\mathrm{cm}$，而$R(72\,\mathrm h)=1.198000\,\mathrm{cm}$；145个测量时刻的$\mathrm{MAE}=0.109782\,\mathrm{cm}$、$\mathrm{RMSE}=0.120537\,\mathrm{cm}$，除初始点外始终有$R_{\mathrm{pred}}(t)>R(t)$，最大误差$0.229165\,\mathrm{cm}$出现在$6\,\mathrm h$，该模型的临界时刻为$56.2103\,\mathrm h$。其中，MAE为平均绝对误差，表示预测半径与实测半径的平均偏离程度；RMSE为均方根误差，对较大的半径偏差更为敏感；$R_{\mathrm{pred}}(t)>R(t)$则表示预测半径始终大于对应的实测半径。

如图\ref{fig:q4-radius-validation}所示，严格守恒模型系统性高估药材半径，即低估了实际径向收缩程度。该单向偏差及其误差量级均远大于数值离散误差，不能归因于求解器精度。附件2半径数据作为可信观测，图中对比结果表明，固定长度下严格恒定干质量的反设无法再现实测径向收缩过程，因而该反设不合理，并从观测一致性角度支持问题四主模型中有效干质量减少的结论。

\begin{figure}[H]
\centering
\includesvg[width=0.94\textwidth,inkscapelatex=false]{figures/固定长度守恒模型的半径预测与附件2实测值比较}
\caption{固定长度守恒模型的半径预测与附件2实测值比较}
\label{fig:q4-radius-validation}
\end{figure}

\subsubsection{干质量减少的数理证明}

上述结论还可不依赖模型的数值分析而直接证明。由式\eqref{eq:q4-check-dry-density}，对$C\ge0$恒有$\rho_d(C)\le760\,\mathrm{kg/m^3}$。若总干质量守恒且长度不超过初始长度，则
\begin{equation}
...
该结论由解析不等式直接得到，不依赖有限体积网格、BDF时间推进或具体求解器，说明主模型得到的有效干质量下降并非数值离散误差所致。


\subsubsection{检验结论与物理解释}

\begin{comment}
主模型及两个严格守恒对照模型的作用与检验结果汇总于表\ref{tab:q4-model-comparison}。主模型提出样品控制体内有效干质量非恒定的结论；两个对照模型则分别从几何合理性和观测一致性两个方向检验严格恒定干质量的反设。

\begin{table}[H]
\centering
\scriptsize
\renewcommand{\arraystretch}{1.25}
\caption{问题四主模型及严格守恒对照模型的检验结果}
\label{tab:q4-model-comparison}
\begin{tabularx}{\textwidth}{|>{\centering\arraybackslash}p{0.11\textwidth}|>{\centering\arraybackslash}p{0.12\textwidth}|>{\centering\arraybackslash}p{0.18\textwidth}|>{\centering\arraybackslash}X|>{\centering\arraybackslash}X|}
\hline
方案 & 干质量设定 & 几何闭合方式 & 主要检验结果 & 证明作用\\
\hline
主模型 & 不强制恒定 & 实测半径、固定长度及均匀仿射收缩 & 正式烘干时间为$51.0906\,\mathrm h$；临界时刻有效干质量相对下降约$10.97\%$ & 提出样品控制体内有效干质量非恒定的待检验结论\\
\hline
守恒对照一 & 强制总量及逐环守恒 & 实测半径、反演长度及非仿射映射 & 最大轴向伸长约$29.54\%$；$47.8764\,\mathrm h$不作为替代答案 & 表明保留实测半径时，严格守恒会产生缺乏合理力学依据的轴向伸长\\
\hline
守恒对照二 & 强制总量及逐环守恒 & 固定长度并独立预测半径 & $\mathrm{RMSE}=0.120537\,\mathrm{cm}$，且除初始点外预测半径始终偏大；$56.2103\,\mathrm h$不作为替代答案 & 表明固定长度时，严格守恒无法再现实测径向收缩\\
\hline
\end{tabularx}
\end{table}
...
对照模型一表明，保留实测半径并强制干质量守恒会导致药材产生显著的轴向伸长；对照模型二表明，保持固定长度并强制干质量守恒会显著降低药材实际径向收缩。两项结果分别从几何合理性和径向收缩一致性否定严格恒定干质量的反设，共同支持主模型中出现的有效干质量下降。
...
\begin{itemize}[leftmargin=2em,itemsep=2pt,topsep=2pt]
  \item \textbf{模型体系层层递进：}四个问题由固定物性、固定区域的传热传质模型，逐步扩展至变物性热湿耦合、长期干燥终止事件定位和收缩区域模型，各问均在前一问的基础上针对新增条件进行修正，具有较好的连贯性。
  \item \textbf{数值方法与问题特征相适应：}采用径向有限体积法离散守恒型控制方程，并利用表层加密的非均匀网格分辨较大的边界梯度；结合后向 Euler、隐式 BDF 和 Picard 迭代，能够稳定处理扩散方程的刚性及变物性引起的非线性耦合。
  \item \textbf{数值结果具有较充分的可靠性检验：}通过空间网格与时间步长收敛性检验、Richardson 误差估计、离散残差检验和临界事件定位，考察数值解的稳定性与精度；进一步采用谱元法和 Radau IIA 方法独立复算，降低结果对单一算法的依赖。
  \item \textbf{实测数据与物理检验结合紧密：}模型利用附件中的动态环境数据构造边界条件，并在问题四中引入实测半径描述药材收缩；同时通过有效干质量计算、解析证明及两个严格守恒对照模型，从几何合理性和观测一致性两方面检验模型结论。
\end{itemize}

\subsection{模型局限}
\begin{itemize}[leftmargin=2em,itemsep=2pt,topsep=2pt]
  \item \textbf{空间结构与收缩形态有所简化：}模型将药材视为均质、各向同性的一维径向连续介质，未描述端面传热传质、轴向梯度、纹理各向异性及分层物性；问题四采用固定长度和均匀仿射径向收缩，也难以反映局部压实、翘曲和开裂等非均匀变形。
  \item \textbf{长期环境边界存在近似误差：}问题三和问题四将第$3$--$4\,\mathrm h$环境数据的平均值作为$4\,\mathrm h$后的常值边界，并保持换热系数和传质系数不变，因而所得长期烘干时间会受到后期环境近似稳定假设的影响。
  \item \textbf{缺少独立实验数据的直接验证：}内部温度场、含水率场及有效干质量降幅主要由模型计算得到，尚缺少内部测点、同步质量与长度测量以及逸出物收集数据，无法进一步辨识挥发、碎屑脱落和固形物渗出等机制的实际贡献。
  \item \textbf{部分热湿传递机制未显式考虑：}温度方程未显式考虑水分蒸发吸收的汽化潜热，也未考虑体热源、化学反应热、辐射换热及热扩散引起的直接水分通量，相关影响作为模型误差处理。
\end{itemize}

\subsection{进一步改进方向}
后续研究可延长烘房环境数据的监测时段，并同步测量药材内部温度、含水率、半径、长度和总质量，以标定变物性参数及表面传递系数，验证长期边界延拓和有效干质量变化。还可收集挥发物、表层碎屑和渗出固形物，进一步辨识干质量损失的主要来源。在此基础上，引入轴向应变、孔隙率及非均匀收缩等状态变量，建立二维轴对称热湿力耦合模型，并将水分蒸发潜热纳入能量方程，以提高模型对真实温度变化、干燥速率和收缩形态的描述能力。
...
\fbox{%
  ...
  \vspace{0.8em}
  \begin{itemize}[leftmargin=2em,itemsep=0.35em,topsep=0pt]
    \item 数据预处理与环境边界插值结果；
    \item 问题一固定物性热湿传递模型程序及结果；
    \item 问题二变物性热湿耦合模型程序及结果；
    \item 问题三长期烘干时间计算程序及结果；
    \item 问题四变半径模型程序及结果；
    \item 模型检验、收敛性分析和结果绘图文件；
    \item AI工具使用详情。
  \end{itemize}
  \vspace{0.4em}
  \centering\itshape 此处可替换为最终支撑材料目录截图
  \vspace{0.8em}
  \end{minipage}%
}
\caption{支撑材料目录}
\label{fig:supporting-materials}
\end{figure}
