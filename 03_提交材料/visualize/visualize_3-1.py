...
chinese_font_path = next(
    (font_path for font_path in CHINESE_FONT_CANDIDATES if font_path.exists()),
    None,
)
if chinese_font_path is not None:
    font_manager.fontManager.addfont(str(chinese_font_path))
    text_font_family = font_manager.FontProperties(fname=str(chinese_font_path)).get_name()
else:
